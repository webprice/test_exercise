...
def test_put_user(id,username,email,password):
...
def test_put_usernot_exist_in_db(id,username,email,password):
    pload = {"username": username,"email":email, "password": password}
    response = requests.put(f"http://127.0.0.1:8000/user/{id}",json = pload)
    print(json.dumps(response.json(), indent=4, sort_keys=True))
...
def test_put_user_wrong_data(id,username,email,password):
    pload = {"username": username,"email":email, "password": password}
    response = requests.put(f"http://127.0.0.1:8000/user/{id}",json = pload)
    print(json.dumps(response.json(), indent=4, sort_keys=True))
