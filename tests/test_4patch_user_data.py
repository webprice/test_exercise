...
def test_patch_user(client,id,username,email,password):
...
def test_patch_usernot_exist_in_db(client,id,username,email,password):
    pload = {"username": username,"email":email, "password": password}
    response = client.patch(f"/user/{id}",json = pload)
    print(json.dumps(response.json(), indent=4, sort_keys=True))
...
def test_patch_user_wrong_data(client,id,username,email,password):
    pload = {"username": username,"email":email, "password": password}
    response = client.patch(f"/user/{id}",json = pload)
    print(json.dumps(response.json(), indent=4, sort_keys=True))
...
def test_patch_user_same_data(client,id,username,email,password):
    pload = {"username": username,"email":email, "password": password}
    response = client.patch(f"/user/{id}",json = pload)
    print(json.dumps(response.json(), indent=4, sort_keys=True))
    assert response.status_code == 406
