...
async def user_post(user: RegisterIn):
    try:
        ...
        await Crud.create(id,user,hashed_password)
    ...
    #eturn await database.fetch_one(check_query)
    #print(Crud.read_by_email(user.email))
    return await Crud.read_by_email(user.email)
